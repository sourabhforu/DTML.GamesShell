  ...
  async submit() {
    ...
    try {
      if (this.state.screenshotChecked) {
        var respondToPostmessage1 = async function(e) 
        { 
          console.log("game origin:" + e.origin);
        ...
        if (this.addedListener === false)
        {
          window.addEventListener('message', respondToPostmessage1); 
          this.addedListener = true;
        }
        (document.getElementsByClassName('gameframe')[0]).contentWindow.postMessage("whats up", "*");
